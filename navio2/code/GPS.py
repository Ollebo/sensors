...
if __name__ == "__main__":
    ...
    while True:
        ...
        if msg.name() == "NAV_POSLLH":
            ...
            print(msg)
            #outstr = "".join(outstr)
            long = outstr[0].split('=')[1]
            lat = outstr[1].split('=')[1]
            jsonGeoRun ={
            'longitude': float(long[:2]+"."+long[2:]),
            'latitude': float(lat[:2]+"."+lat[2:]),
            'height': int(outstr[2].split('=')[1]),
            'hMSL': int(outstr[3].split('=')[1]),
            'hAcc': int(outstr[4].split('=')[1]),
            'vAcc': int(outstr[5].split('=')[1]),
            }
            jsonGEO = jsonGeoRun
        ...
        if msg.name() == "NAV_STATUS":
            ...
            outstr = "".join(outstr)
            jsonGEO['status']=int(outstr.split("=")[1])
        if 'vAcc' in jsonGEO.keys():
            writeStatus(jsonGEO)
            print(jsonGEO)
